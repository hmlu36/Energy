    ...
    public class Page5 : Base
    {
        public void CreateRow (DatabaseSearchPage viewModel, DataRow _row, DataColumn Header, FlowModel FlowRow, EnergyModel EnergyRow, string 欄位名)
        {
            ...
            if ((string)_row["年季月"] == "M")
            {
                ...
                var mlist = viewModel.FuelData_Db.Where(p => p.yr_mnth == s1).ToList();

                
                foreach (var colId in FlowRow.ColdIdAry)
                {
                    
                    string new欄位名 = colId + 欄位名;
                    foreach (var m in mlist)
                    {
                        AddRow<DbContext.fuel50_db>(_row, m, Header.ColumnName, new欄位名);
                    }
                }
            }
            else if ((string)_row["年季月"] == "Y")
            {
                ...
                var mlist = viewModel.FuelData_Db.Where(p => yr_myth_list.Contains(p.yr_mnth));
